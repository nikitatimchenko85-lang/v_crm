...
var COLS = [
  ...
  ['clientLinkId','Ссылка клиенту'], ['firstContactAt','Первый контакт']
];

/* Таблица — резервная копия, а не рабочая: по названию из папки бэкапов или по листу «⚠ АРХИВ».
 * Копия не должна принимать заявки и синхронизацию — иначе данные уходят «в никуда». */
function isBackupCopy() {
  var name = SpreadsheetApp.getActiveSpreadsheet().getName();
  return isArchive() || /^(АРХИВ\s+)?CRM\s+\d{4}-\d{2}-\d{2}$/.test(name) || /^(Копия|Copy of)\s/i.test(name);
}
function refuseBackup() {
  return out({status:'error', message:'Эта ссылка ведёт в резервную копию таблицы «' +
    SpreadsheetApp.getActiveSpreadsheet().getName() + '». Разверните скрипт в рабочей таблице и обновите ссылку.'});
}

function doPost(e) {
  if (isBackupCopy()) return refuseBackup();
  ...
  try {
    ...
    hide(sh, ['id','statusId','venueId','payments','history','party','flags','plan','clientLinkId']);
    ...
    try { writePricingSheet(keepSettings.settings); } catch (prcErr) { Logger.log('Прайс: ' + prcErr); }
    try { writeCabinetSheet(body.deals || []); } catch (cabErr) { Logger.log('Кабинет: ' + cabErr); }
    try { writeCabinetData(body.deals || [], body.rentals || []); } catch (cdErr) { Logger.log('Кабинет-данные: ' + cdErr); }
...
/* занятость и прайс для публичной страницы — только то, что и так видно на сайте, без клиентов и телефонов */
function publicData() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  function rows(name) {
    var sh = ss.getSheetByName(name);
    return sh && sh.getLastRow() ? sh.getDataRange().getDisplayValues() : [];
  }
  return out({status:'ok', busy: rows(BUSY_SHEET), pricing: rows(PRICING_SHEET)});
}

function doGet(e) {
  try {
    if (isBackupCopy()) return refuseBackup();
    if (e.parameter && e.parameter.hook === 'public') return publicData();   // до отладки: каждый просмотр сайта не пишем в лог
    if (e.parameter && e.parameter.hook) return handleHook(e.parameter.hook, e);
...
/* ---------- лист «Прайс» для калькулятора сметы ----------
 * Только цены и вместимость — ничего о конкретных клиентах.
 * Формат многострочный: первая колонка kind различает залы, территорию целиком и прокат. */
var PRICING_SHEET = 'Прайс';

function writePricingSheet(settingsJson) {
  var s = {};
  try { s = JSON.parse(settingsJson || '{}'); } catch (e) { return 0; }
  var venues = s.venues || [];
  var whole = s.wholePrice || {};
  var catalog = s.catalog || [];

  var head = ['kind', 'id', 'name', 'cap', 'color', 'priceWeek', 'priceWeekend', 'plotWeek', 'plotWeekend', 'cat', 'unit', 'price'];
  var rows = [head];

  venues.forEach(function(v) {
    rows.push(['venue', v.id || '', v.name || '', v.cap || '', v.color || '',
      v.priceWeek || 0, v.priceWeekend || 0, v.plotWeek || 0, v.plotWeekend || 0, '', '', '']);
  });
  rows.push(['whole', '', 'Вся территория', '', '', whole.week || 0, whole.weekend || 0, '', '', '', '', '']);
  catalog.forEach(function(c) {
    if (!c.price) return;                       // бесплатные позиции клиенту показывать незачем
    rows.push(['extra', c.id || '', c.name || '', '', '', '', '', '', '', c.cat || '', c.unit || 'шт', c.price || 0]);
  });

  var sh = sheet(PRICING_SHEET);
  sh.clear();
  sh.getRange(1, 1, rows.length, head.length).setValues(rows);
  return rows.length - 1;
}

function publishPricing() {
  refuseInArchive();
  var n = writePricingSheet(readBlobs().settings || '{}');
  var msg = 'Лист «Прайс» заполнен, строк: ' + n +
            '. Опубликуйте его: Файл → Поделиться → Опубликовать в интернете → «Прайс» → CSV.';
  Logger.log(msg);
  return msg;
}

/* ---------- лист «Кабинет» для личной страницы клиента ----------
 * Публикуется только для заявок, где в CRM нажали «Создать ссылку для клиента» —
 * то есть только те, у кого реально есть эта ссылка, а не все подряд. */
var CABINET_SHEET = 'Кабинет';
...
function writeCabinetSheet(deals) {
  var head = ['token', 'client', 'date', 'hall', 'time', 'guests', 'amount', 'paid', 'due', 'contract'];
  var rows = [head];
  deals.forEach(function(d) {
    var token = String(d.clientLinkId || '');
    if (!token) return;                          // ссылку не создавали — в публикацию не попадает
    var whole = d.whole === true || d.whole === 'да';
    var amount = (Number(d.amount) || 0) + (Number(d.plotAmount) || 0);
    var paid = Number(d.paid) || 0;
    var ts = cleanTime(d.timeStart), te = cleanTime(d.timeEnd);   // время из таблицы ненадёжно (см. cleanTime)
    var time = ts ? (ts + (te ? '–' + te : '')) : '';
    rows.push([token, firstNameOf(d.client), ymdOf(d.date),
      whole ? 'Вся территория' : String(d.venue || ''), time, d.guests || '',
      amount, paid, Math.max(0, amount - paid), d.contractNo ? 'подписан' : 'не оформлен']);
  });

  var sh = sheet(CABINET_SHEET);
  sh.clear();
  sh.getRange(1, 1, rows.length, head.length).setValues(rows);
  if (rows.length > 1) sh.getRange(2, 3, rows.length - 1, 1).setNumberFormat('@');
  return rows.length - 1;
}

function publishCabinet() {
  refuseInArchive();
  var n = writeCabinetSheet(readDealObjs());
  var msg = 'Лист «Кабинет» заполнен, ссылок: ' + n +
            '. Опубликуйте его: Файл → Поделиться → Опубликовать в интернете → «Кабинет» → CSV.';
  Logger.log(msg);
  return msg;
}

/* ================= ЖИВОЙ КАБИНЕТ КЛИЕНТА ================= */
/*
 * В отличие от листа «Кабинет» выше (публичный CSV, только для обзора вручную),
 * данные ниже НИКОГДА не публикуются в интернет. Личная страница клиента получает
 * их только через прямой запрос к скрипту — по токену из ссылки или по email+паролю
 * после входа. Так по одной открытой ссылке нельзя скачать сразу всех клиентов разом.
 *
 * КабинетДанные: [token, json]           — по одной строке на заявку со ссылкой
 * Вход:          [email, hash, token]    — учётные записи, e-mail в нижнем регистре
 */
var CABINET_DATA_SHEET = 'КабинетДанные';
var LOGIN_SHEET = 'Вход';

function cabinetPayload(d, rentals) {
  var whole = d.whole === true || d.whole === 'да';
  var amount = (Number(d.amount) || 0) + (Number(d.plotAmount) || 0);
  var paid = Number(d.paid) || 0;
  var ts = cleanTime(d.timeStart), te = cleanTime(d.timeEnd);
  var myRentals = (rentals || [])
    .filter(function(r) { return String(r.dealId || '') === String(d.id || '') && r.status !== 'canceled'; })
    .map(function(r) { return r.itemsText || ''; })
    .filter(String);
  var plan = {tables: []};
  try { plan = JSON.parse(d.plan || '{"tables":[]}'); } catch (e) {}

  return {
    client: firstNameOf(d.client),
    date: ymdOf(d.date),
    hall: whole ? 'Вся территория' : String(d.venue || ''),
    whole: whole,
    time: ts ? (ts + (te ? '–' + te : '')) : '',
    guests: d.guests || '',
    amount: amount, paid: paid, due: Math.max(0, amount - paid),
    contract: d.contractNo ? 'подписан' : 'не оформлен',
    rentals: myRentals,
    plan: plan
  };
}

/* пишется при каждой синхронизации — быстрый снимок для мгновенной выдачи по хуку */
function writeCabinetData(deals, rentals) {
  // то, что уже лежит в кабинете, — читаем перед перезаписью, чтобы ничего не потерять
  var prior = {};
  var oldSh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CABINET_DATA_SHEET);
  if (oldSh && oldSh.getLastRow() > 1) {
    oldSh.getRange(2, 1, oldSh.getLastRow() - 1, 2).getValues().forEach(function(r) {
      try { prior[String(r[0])] = JSON.parse(r[1]); } catch (e) {}
    });
  }

  var rows = [['token', 'json']], seen = {};
  deals.forEach(function(d) {
    var token = String(d.clientLinkId || '');
    if (!token) return;
    seen[token] = true;
    var payload = cabinetPayload(d, rentals);

    // гостей на местах вписывает клиент сам — обычная синхронизация из CRM не должна их стирать
    var priorNames = {};
    var pTables = prior[token] && prior[token].plan && prior[token].plan.tables;
    if (pTables) pTables.forEach(function(t) {
      if ((t.names || []).some(function(x) { return String(x || '').trim(); })) priorNames[t.id] = t.names;
    });
    if (payload.plan && payload.plan.tables) payload.plan.tables.forEach(function(t) {
      var already = (t.names || []).some(function(x) { return String(x || '').trim(); });
      if (!already && priorNames[t.id]) t.names = priorNames[t.id];
    });

    rows.push([token, JSON.stringify(payload)]);
  });

  // заявки, отправленные с сайта, но ещё не открытые менеджером как настоящая сделка, — сохраняем как есть
  Object.keys(prior).forEach(function(token) {
    if (seen[token]) return;
    if (prior[token] && prior[token].pending) rows.push([token, JSON.stringify(prior[token])]);
  });

  var sh = sheet(CABINET_DATA_SHEET);
  sh.clear();
...
function findCabinetRow(token) {
  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CABINET_DATA_SHEET);
...
function hasLoginFor(token) {
  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LOGIN_SHEET);
  if (!sh || sh.getLastRow() < 2) return false;
  var vals = sh.getRange(2, 1, sh.getLastRow() - 1, 3).getValues();
  for (var i = 0; i < vals.length; i++) if (String(vals[i][2]) === String(token)) return true;
  return false;
}

function passHash(email, password) {
  var raw = String(email).trim().toLowerCase() + ':' + String(password);
  var bytes = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, raw, Utilities.Charset.UTF_8);
  return Utilities.base64Encode(bytes);
}

function handleCabinetLookup(p) {
  var token = pick(p, ['token', 'id']);
  if (!token) return out({status: 'error', message: 'нет ссылки'});
  var row = findCabinetRow(token);
  if (row < 0) return out({status: 'error', message: 'не найдено'});
  var json = sheet(CABINET_DATA_SHEET).getRange(row, 2).getValue();
  var payload;
  try { payload = JSON.parse(json); } catch (e) { return out({status: 'error', message: 'повреждённые данные'}); }
  payload.status = 'ok';
  payload.hasLogin = hasLoginFor(token);
  return out(payload);
}

function handleRegister(p) {
  var token = pick(p, ['token', 'id']);
  var email = pick(p, ['email']).trim().toLowerCase();
  var password = pick(p, ['password']);
  if (!token || !email || password.length < 4) {
    return out({status: 'error', message: 'заполните e-mail и пароль (от 4 символов)'});
  }
  if (findCabinetRow(token) < 0) return out({status: 'error', message: 'ссылка недействительна'});

  var sh = sheet(LOGIN_SHEET);
  if (sh.getLastRow() === 0) {
    sh.getRange(1, 1, 1, 3).setValues([['email', 'hash', 'token']]).setFontWeight('bold').setBackground('#ECEEEB');
    sh.setFrozenRows(1);
    sh.hideSheet();
  }
  var hash = passHash(email, password);
  var vals = sh.getLastRow() > 1 ? sh.getRange(2, 1, sh.getLastRow() - 1, 3).getValues() : [];
  var updated = false;
  for (var i = 0; i < vals.length; i++) {
    if (String(vals[i][2]) === String(token)) {                 // повторная попытка по той же ссылке — сброс пароля
      sh.getRange(i + 2, 1, 1, 2).setValues([[email, hash]]);
      updated = true; break;
    }
    if (String(vals[i][0]) === email) {
      return out({status: 'error', message: 'этот e-mail уже занят другой заявкой'});
    }
  }
  if (!updated) sh.appendRow([email, hash, token]);

  return handleCabinetLookup({token: token});
}

function handleLogin(p) {
  var email = pick(p, ['email']).trim().toLowerCase();
  var password = pick(p, ['password']);
  if (!email || !password) return out({status: 'error', message: 'введите e-mail и пароль'});

  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LOGIN_SHEET);
  if (!sh || sh.getLastRow() < 2) return out({status: 'error', message: 'неверный e-mail или пароль'});
  var vals = sh.getRange(2, 1, sh.getLastRow() - 1, 3).getValues();
  var hash = passHash(email, password);
  for (var i = 0; i < vals.length; i++) {
    if (String(vals[i][0]) === email && String(vals[i][1]) === hash) {
      return handleCabinetLookup({token: vals[i][2]});
    }
  }
  return out({status: 'error', message: 'неверный e-mail или пароль'});
}

/* ---------- восстановление пароля по e-mail ---------- */
/* Пароль не хранится как есть (только хеш), поэтому «вспомнить» его нельзя —
 * можно только выслать свежую ссылку на кабинет, а войти по ней клиент задаст новый пароль.
 * Перед первым использованием впишите в Настройки проекта → Свойства скрипта
 * значение SITE_URL — адрес сайта с конечным «/», например
 * https://nikitatimchenko85-lang.github.io/v_crm/ */
function handleResetRequest(p) {
  var email = pick(p, ['email']).trim().toLowerCase();
  var okMsg = 'Если такой e-mail зарегистрирован, мы выслали на него ссылку.';
  if (!email) return out({status: 'error', message: 'укажите e-mail'});

  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(LOGIN_SHEET);
  var token = '';
  if (sh && sh.getLastRow() > 1) {
    var vals = sh.getRange(2, 1, sh.getLastRow() - 1, 3).getValues();
    for (var i = 0; i < vals.length; i++) if (String(vals[i][0]) === email) { token = String(vals[i][2]); break; }
  }
  if (!token) { debugHook('сброс пароля', {email: email, найдено: false}); return out({status: 'ok', message: okMsg}); }

  var base = PropertiesService.getScriptProperties().getProperty('SITE_URL');
  if (!base) {
    debugHook('сброс пароля', {email: email, ошибка: 'SITE_URL не задан в свойствах скрипта'});
    return out({status: 'error', message: 'На сервере не настроен адрес сайта (SITE_URL). Сообщите администратору.'});
  }
  var link = base.replace(/\/?$/, '/') + 'client.html?id=' + encodeURIComponent(token);
  try {
    MailApp.sendEmail({
      to: email,
      subject: 'Ссылка на ваш личный кабинет — Villatic',
      body: 'Здравствуйте!\n\nВаша ссылка для входа в личный кабинет мероприятия:\n' + link +
            '\n\nПо ней можно задать новый пароль. Если вы не запрашивали эту ссылку, просто проигнорируйте письмо.'
    });
    debugHook('сброс пароля', {email: email, отправлено: true});
  } catch (err) {
    debugHook('сброс пароля', {email: email, ошибка: String(err)});
    return out({status: 'error', message: 'Не удалось отправить письмо. Попробуйте позже или напишите нам.'});
  }
  return out({status: 'ok', message: okMsg});
}

/* клиент сам вписывает гостей на места — сохраняем только имена, всё остальное в плане не трогаем */
function applyNames(plan, incoming) {
  var byId = {};
  (incoming || []).forEach(function(t) { byId[t.id] = t.names || []; });
  (plan.tables || []).forEach(function(t) {
    if (byId.hasOwnProperty(t.id)) {
      t.names = byId[t.id];
      t.guests = t.names.filter(function(x) { return String(x || '').trim(); }).length || t.guests;
    }
  });
  return plan;
}

function handleSeatingSave(p) {
  var token = pick(p, ['token', 'id']);
  var tablesJson = pick(p, ['tables']);
  if (!token || !tablesJson) return out({status: 'error', message: 'нет данных'});
  var incoming;
  try { incoming = JSON.parse(tablesJson); } catch (e) { return out({status: 'error', message: 'повреждённые данные'}); }

  var cdRow = findCabinetRow(token);
  if (cdRow < 0) return out({status: 'error', message: 'ссылка недействительна'});

  // структуру столов берём из настоящей сделки, если она уже есть, — там её строил сотрудник.
  // В приватном снимке кабинета структуры может ещё не быть (заявка только что с сайта).
  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET);
  var linkCol = COLS.map(function(c) { return c[0]; }).indexOf('clientLinkId') + 1;
  var planCol = COLS.map(function(c) { return c[0]; }).indexOf('plan') + 1;
  var dealRow = -1, plan = null;

  if (sh && linkCol && planCol && sh.getLastRow() > 1) {
    var links = sh.getRange(2, linkCol, sh.getLastRow() - 1, 1).getValues();
    for (var i = 0; i < links.length; i++) {
      if (String(links[i][0]) === String(token)) { dealRow = i + 2; break; }
    }
  }
  if (dealRow > 0) {
    try { plan = JSON.parse(sh.getRange(dealRow, planCol).getValue() || '{"tables":[]}'); } catch (e2) { plan = {tables: []}; }
  } else {
    var cdSheet0 = sheet(CABINET_DATA_SHEET);
    var cur = {};
    try { cur = JSON.parse(cdSheet0.getRange(cdRow, 2).getValue()); } catch (e3) {}
    plan = cur.plan || {tables: []};
  }

  plan = applyNames(plan, incoming);
  if (dealRow > 0) sh.getRange(dealRow, planCol).setValue(JSON.stringify(plan));   // сотруднику видно в CRM

  var cdSheet = sheet(CABINET_DATA_SHEET);
  var payload = {};
  try { payload = JSON.parse(cdSheet.getRange(cdRow, 2).getValue()); } catch (e4) {}
  payload.plan = plan;
  cdSheet.getRange(cdRow, 2).setValue(JSON.stringify(payload));
...
function handleHook(kind, e) {
  ...
  if (need && kind === 'call' && got !== need) {   // секрет нужен только АТС: код сайта публичный, секрет в нём не спрятать
  ...
  if (kind === 'register') return handleRegister(p);
  if (kind === 'login') return handleLogin(p);
  if (kind === 'cabinet') return handleCabinetLookup(p);
  if (kind === 'seating') return handleSeatingSave(p);
  if (kind === 'reset') return handleResetRequest(p);
...
function debugHook(kind, p) {
  try {
    ...
    var safe = {};
    for (var k in p) safe[k] = /pass|token|секрет/i.test(k) && p[k] ? '***' : p[k];
    sh.appendRow([new Date(), kind, JSON.stringify(safe).slice(0, 5000)]);
...
function saveLead(p) {
  var sh = sheet(LEADS);
  if (sh.getLastRow() === 0) {
    sh.getRange(1, 1, 1, 13).setValues([['Время','Имя','Телефон','Контакт','Дата события','Гостей','Комментарий','Источник','id','Статус','Токен кабинета','E-mail','Зал']])
      .setFontWeight('bold').setBackground('#ECEEEB');
    sh.setFrozenRows(1);
  }
  ...
  var date = pick(p, ['date','event_date']);
  var guests = pick(p, ['guests','pax']);
  var comment = pick(p, ['comment','message','text']);
  var source = pick(p, ['source','utm_source']) || 'Сайт';
  var email = pick(p, ['email']).trim().toLowerCase();
  var password = pick(p, ['password']);
  var hall = pick(p, ['hall']);
  var plot = pick(p, ['plot']) === 'true';

  // бронирование с сайта: клиент сразу получает доступ в свой кабинет,
  // не дожидаясь, пока менеджер откроет CRM и заявка станет настоящей сделкой
  var token = '';
  if (email && password) {
    token = Utilities.getUuid() + Utilities.getUuid();
    var payload = {
      client: firstNameOf(name), date: date, hall: hall, whole: hall === 'Вся территория', plot: plot,
      time: '', guests: guests, amount: 0, paid: 0, due: 0, contract: 'не оформлен',
      rentals: [], plan: {tables: []}, pending: true
    };
    var cdSheet = sheet(CABINET_DATA_SHEET);
    if (cdSheet.getLastRow() === 0) { cdSheet.getRange(1, 1, 1, 2).setValues([['token', 'json']]); cdSheet.hideSheet(); }
    cdSheet.appendRow([token, JSON.stringify(payload)]);

    var loginSh = sheet(LOGIN_SHEET);
    if (loginSh.getLastRow() === 0) {
      loginSh.getRange(1, 1, 1, 3).setValues([['email', 'hash', 'token']]).setFontWeight('bold').setBackground('#ECEEEB');
      loginSh.setFrozenRows(1); loginSh.hideSheet();
    }
    loginSh.appendRow([email, passHash(email, password), token]);
  }

  sh.appendRow([new Date(), name, phone, pick(p, ['contact','telegram']), date, guests, comment, source, id, 'новая', token, email, hall]);
  tgSendAll([{text:'Новая заявка: ' + name + ' ' + phone + (token ? ' (с личным кабинетом)' : '')}]);
  return out({status:'ok', id:id, token:token});
}
...
function readLeads() {
  ...
  vals.forEach(function(r) {
    ...
    res.push({id:String(r[8]), name:String(r[1]), phone:String(r[2]), contact:String(r[3]),
      ...
      clientLinkId:String(r[10] || ''), hall:String(r[12] || '')});
  });
  return res;
}
